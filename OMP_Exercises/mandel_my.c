...
#pragma omp parallel for reduction(+:tp) default(none) private(j, creal, cimag) shared(eps)
   for (i=0; i<NPOINTS; i++) {
     for (j=0; j<NPOINTS; j++) {
       creal = -2.0+2.5*(double)(i)/(double)(NPOINTS)+eps;
       cimag = 1.125*(double)(j)/(double)(NPOINTS)+eps;
       tp += testpoint(creal, cimag);
     }
   }

// Calculate area of set and error estimate and output the results
   
area=2.0*2.5*1.125*(double)(NPOINTS*NPOINTS-tp)/(double)(NPOINTS*NPOINTS);
//area=2.0*2.5*1.125*(double)(NPOINTS*NPOINTS-numoutside)/(double)(NPOINTS*NPOINTS);
   error=area/(double)NPOINTS;
   double actual = 1.510659;
   double diff = area-actual;
   printf("Area of Mandlebrot set = %12.8f +/- %12.8f\n",area,error);
   printf("Correct answer should be around %12.8f \n", actual);
   printf("Area difference is %12.8f \n",diff);
